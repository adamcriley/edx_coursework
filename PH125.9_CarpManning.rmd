...
\
This project will focus on assisting legal practitioners to predict the outcome of cases with social impact components. This litigation is often costly and can take years to complete. With this information, applied to the facts of their particular issue, they can either temper the expectations of a plaintiff with a case that will not likely be ruled in their favor or to be more aggressive with a defendant that is facing a potential loss and is looking for a way to mitigate the impact of the loss.\
\
...
Independent variables include details about the judge writing the opinion, information about the court, topical classifications about the subject matter of the case, and the date of ruling. Each of the independent variables has been encoded but can be decoded using the above referenced companion codebook. We will look further into each of the independent variables as we inspect and process the data below.\
...
\
There are a lot of other additional classifiers that could be added to this data to improve the fit and accuracy of the model. States could be assigned different regions (which may could bet distinct from circuits), case types could be further sub-divided based on topic or assigned into sub-categories, more demographic information could be added about the judges. These steps could add value, especially subdividing the topics, and should be explored if the model would have a commercial application which would require a very high degree of accuracy based on the business case (as described above).\
```{r include=FALSE}
...
glimpse(CMtraining)
```
The training set consists of 94,340 entries of 30 columns (11 of which are duplicate and decoded). The average case is outcome is a conservative one (~58% of the time). With 19 independent variables to consider, we need to visually inspect how our outcome is being influenced. Many of our independent variables are related, so we will briefly look at each group.\
...
rm(tend_party)
```
There were 4 designations provided by the authors for this characteristic: "Democrat" ("Dem"), "Republican" ("Rep"), "Independent/Other/Unknown" ("Ind"), and "99". I am not sure, nor do the authors indicate what the the "99" designation is associated with, but I removed the factor from the chart.\
\
This display matches with expectations almost perfectly with Democrats being traditionally being perceived as more liberal and Republican as being more conservative over this time frame. Additionally, Independents seem not to follow a specific trend which is also an expected pattern. These trends make common sense as political party is a feature chosen by the judge and it logically follows that the expression of the political party would also provide good information about the ruling tendencies of each judge.\
\newpage
...
rm(tend_pres)
```
This visualization is too crowded to be able to make sense of and no trends are apparent from the quick examination. The are so many presidents listed that adding trend lines makes the graph more confusing and even removing the data points all together and just leaving the lines was still too much. We should peel the information back a layer and group the presidents by their political party and see if a trend emerges. It feels like there should be some sort of correlation, as presidents would be more likely to appoint judges that have views similar to their own:\
```{r echo=FALSE}
# pres by party
...
rm(tend_pres)
```
We can now see a clear trend in the data and the shape of our trend lines mirrors the trend lines of the judge's political party. This also makes common sense and tells us that the appointing president designation might be conflated. We need to be caution of that when examining our model later.\
\newpage
...
rm(treeMap)
```
The tree diagram is useful here because it allows us to take a look at the variance from each of the 31 case type designations while keeping them grouped within the 3 case type categories. There are noticeable trend here, which is expected, and if we view the categories as a whole over time, we can see that they seem to have a stronger effect:\
```{r echo=FALSE}
# category
...
rm(trend_cat)
```
The case type categories almost form bands, which meets our anecdotal expectations, with judges treated economic and labor matters much more liberally than criminal justice issues.\

### Limitations  

One final point before we move on. Legal cases involve one or more plaintiffs against one or more defendants contesting a specific factual pattern. While there are obviously trends in the data based on factors outside of the case itself, at the end of the day we will not be able to always determine the outcome of a case. This unknown variability needs to be taken into account when fitting our model.\
\
We also mentioned confounding earlier, but many of the independent variables may be related or may seem to be associated when underlying factors may be the cause of both the presentation in the dependent and independent variables. We need to cautious when examining our model and using it because of this.\
\
While I initially intended to use Linear discriminant analysis (LDA) as one of the models for this project, the data does not fit several of the assumptions required, which is worth mentioning here. Firstly, the data independent variables do not seem to be normally distributed, with much of the data skewed towards one of the classifiers. Additionally, some of the data sets that contain specific classifiers are too small for the model to function, because there are less data points than degrees of freedom in the model.\
\
I had also originally also wanted to use a k-nearest neighbor approach, but the categorical nature of the data and small number of selected factors compared to the amount of data meant that the distance function I was using to calculate the neighborhood was too saturated and would fail, even with a neighborhood at $k=1$.\ 

## Data Analysis Conslusions

All of the features we examined look promising as predictor so we will select them all except the judge's political party, which we discussed may be too correlated with other predictors. We will use 8 predictors:\
...
Using the same ideas we discussed when we set aside a portion of our data to do unbiased testing, we can use a similar approach to when working to tune our model parameters. We can examine the training data and randomly use 85% of the remaining data, model it and test it against the 15% of data that was left over. When the first pass is completed, we can replace the sample we used and sample again. This will allow us to find parameters that are more stable and robust than just modeling against the entire training set at once. We will use random selection for the sample each time, using a method called "bootstrapping." For these models we will randomly sample with replacement 10 times. This control method is defined and used for tuning in all of our models for consistency.\
```{r}
...
\
Because we have a Bernoulli distribution, we can use a special case of the linear model called logistical Regression. Logistical regression is another popular variation of the general linear model specifically used to classify binary events, like the outcome of a case being liberal or conservative. as we have here.\
\
...
An ensemble model uses predictions from other models to make its prediction. The model looks at all the other predictions and then uses a majority voting approach to determine how it should predict the outcome. If a majority of the models predicts the outcome of the case as liberal, the ensemble will classify it that was well. We are using three underlying models and do not need to consider a tie breaker mechanism. We can view the model this way:\
...
Before we can examine how our models performed, we need to define the metric we will use to compare them. A reason that the legal industry has bee slow to adopt artificial intelligence and machine learning is that users expect very accurate predictions and the business will not compromise or allow for a general weakness within models.\
\
...
Great! We've improved over the linear fit and have a new candidate for best fit.\
...
Our goal was to create a classifier that predicted if a case would have liberal or conservative ruling and we were able to build several models that performed moderately when looking a robust metric utilizing a handful of diverse variables in the Carp-Manning data set, although we faced several limitations that we outlined above. Our best performing model was the random forest. \
\
